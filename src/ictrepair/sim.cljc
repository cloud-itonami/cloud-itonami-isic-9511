(ns ictrepair.sim
  ...
  media-sanitization screening -> repair completion (escalate/approve/
  commit) -> device return (escalate/approve/commit), then shows HARD-
  hold scenarios: a jurisdiction with no spec-basis, a parts-cost
  mismatch (verified first), a failed post-repair safety test
  (screened directly via `:safety/screen`), an unconfirmed media-
  sanitization status on a storage-replacement ticket (screened
  directly via `:media/screen` -- never via an actuation op against an
  ...
  `socialresearch`'s, `bizassoc`'s, `vocational`'s, `training`'s,
  `furniture`'s, `specialtyrepair`'s and `leathergoods`'s ADR-0001s
  already recorded), a double completion, and a double return."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== ticket/intake ticket-1 (JPN, clean; no storage replacement) ==")
    ...
    (println "== media/screen ticket-1 (no storage replacement; escalates -- human approves) ==")
    (println (exec-op actor "t3b" {:op :media/screen :subject "ticket-1"} operator))
    ...
    (let [r (exec-op actor "t5" {:op :device/return :subject "ticket-1"} operator)]
      (println r)
      (println "-- human repair technician approves --")
    ...
    (println "== repair/complete ticket-3 (claimed 150.0 vs recompute 100.0 -> HARD hold) ==")
    ...
    (println "== media/screen ticket-5 (storage replacement, sanitization unconfirmed -> HARD hold, never reaches a human) ==")
    (println (exec-op actor "t10" {:op :media/screen :subject "ticket-5"} operator))
    ...
    (println (exec-op actor "t12" {:op :device/return :subject "ticket-1"} operator))
