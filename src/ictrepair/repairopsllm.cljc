(ns ictrepair.repairopsllm
  "RepairOps-LLM client -- the *contained intelligence node* for the
  ICT-equipment-repair actor.

  It normalizes ticket intake, drafts a per-jurisdiction consumer-
  product-safety/media-sanitization evidence checklist, screens
  tickets for a failed post-repair safety test, screens tickets for an
  unconfirmed media-sanitization status, drafts the repair-completion
  action, and drafts the device-return action. CRITICAL: it is a
  smart-but-untrusted advisor. It returns a *proposal* (with a
  rationale + the fields it cited), never a committed record or a real
  repair completion/device return. Every output is censored downstream
  by `ictrepair.governor` before anything touches the SSoT, and
  `:repair/complete`/`:device/return` proposals NEVER auto-commit at
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :stake      kw|nil         ; :actuation/complete-repair | :actuation/return-device | nil
     :confidence 0..1}"
...
(defn- assess-jurisdiction
  "Per-jurisdiction consumer-product-safety/media-sanitization
  ...
  NO official spec-basis in `ictrepair.facts` -- the Repair Governor
  ...
  (let [t (store/ticket db subject)
    ...
    (if (nil? sb)
      {:summary    (str iso3 " の公式spec-basisが見つかりません")
       :rationale  "ictrepair.facts に未登録の法域。要件を推測で作らない。"
       :cites      []
...
(defn- screen-safety
  ...
  the ticket record injects the failure mode: the Repair Governor must
  HOLD, un-overridably, on any failed test."
  [db {:keys [subject]}]
  (let [t (store/ticket db subject)]
    (cond
      (nil? t)
...
(defn- screen-media
  "Media-sanitization screening draft -- the genuinely new screening
  concern this vertical adds. `:media-sanitization-confirmed? false`
  on a ticket that itself involves a storage-component replacement
  injects the failure mode: the Repair Governor must HOLD, un-
  overridably, on any unconfirmed media-sanitization status."
  [db {:keys [subject]}]
  (let [t (store/ticket db subject)]
    (cond
      (nil? t)
      {:summary "対象ticketが見つかりません" :rationale "no ticket record"
       :cites [] :effect :media-screening/set :value {:ticket-id subject :verdict :unknown}
       :stake nil :confidence 0.0}

      (not (true? (:involves-storage-replacement? t)))
      {:summary    (str (:customer t) ": 記憶媒体の交換を伴わない -- 消去証明は不要")
       :rationale  "involves-storage-replacement? が false のため、記憶媒体消去証明要件そのものが発生しない。"
       :cites      [:media-determination]
       :effect     :media-screening/set
       :value      {:ticket-id subject :verdict :not-applicable}
       :stake      nil
       :confidence 0.9}

      (not (true? (:media-sanitization-confirmed? t)))
      {:summary    (str (:customer t) ": 記憶媒体消去証明が未完了")
       :rationale  "記憶媒体の交換を伴うが消去証明が未完了。人手確認とホールドが必須。"
       :cites      [:media-check]
       :effect     :media-screening/set
      ...
      :else
      {:summary    (str (:customer t) ": 記憶媒体消去証明済み")
       :rationale  "記憶媒体の交換を伴う、消去証明済み。"
       :cites      [:media-check]
       :effect     :media-screening/set
...
(defn- propose-repair-completion
  ...
  a phase's `:auto` set (`ictrepair.phase`); the governor also always
...
(defn- propose-device-return
  "Draft the actual DEVICE-RETURN action -- returning a real device to
  the customer. ALWAYS `:stake :actuation/return-device` -- this is a
  REAL-WORLD act (an unsafe device, or a device whose removed storage
  media isn't yet sanitized, could reach a customer or a disposal
  stream), never a draft the actor may auto-run. See README
  `Actuation`: no phase ever adds this op to a phase's `:auto` set
  (`ictrepair.phase`); the governor also always escalates on
  `:actuation/return-device`. Two independent layers agree,
  deliberately."
  [db {:keys [subject]}]
  (let [t (store/ticket db subject)
        ready? (and t (:safety-test-passed? t)
                   (or (not (:involves-storage-replacement? t))
                       (:media-sanitization-confirmed? t)))]
    {:summary    (str subject " 向け返却提案"
                      (when t (str " (customer=" (:customer t) ")")))
     :rationale  (if t
                   (str "safety-test-passed?=" (:safety-test-passed? t)
                        " involves-storage-replacement?=" (:involves-storage-replacement? t)
                        " media-sanitization-confirmed?=" (:media-sanitization-confirmed? t))
                   "ticketが見つかりません")
     :cites      (if t [subject] [])
     :effect     :ticket/mark-returned
     :value      {:ticket-id subject}
     :stake      :actuation/return-device
...
(defn infer
  ...
  (case op
    ...
    :media/screen                           (screen-media db request)
    :repair/complete                            (propose-repair-completion db request)
    :device/return                                  (propose-device-return db request)
...
(def ^:private system-prompt
  (str "あなたはコンピュータ・サーバ等のICT機器専門修理店の修理完了・返却エージェントの"
       ...
       ":media-screening/set|:ticket/mark-completed|:ticket/mark-returned) "
       ":stake(:actuation/complete-repair か :actuation/return-device か nil) :confidence(0..1)。\n"
       "重要: 登録されていない法域の要件を絶対に創作してはいけません。"
       "spec-basisが無い場合は :cites を空にし confidence を上げないこと。"
       "記憶媒体消去証明の状況を偽って報告してはいけません。"))

(defn- facts-for [st {:keys [op subject]}]
  (case op
    :jurisdiction/assess    {:ticket (store/ticket st subject)}
    :safety/screen          {:ticket (store/ticket st subject)}
    :media/screen           {:ticket (store/ticket st subject)}
    :repair/complete        {:ticket (store/ticket st subject)}
    :device/return          {:ticket (store/ticket st subject)}
    {:ticket (store/ticket st subject)}))

(defn- parse-proposal
  "Parse the model's EDN proposal defensively. Any parse/shape failure
  yields a safe low-confidence noop so the Repair Governor escalates/
  holds -- an LLM hiccup can never auto-complete a repair or auto-
  return a device."
  [content]
