(ns ictrepair.registry
  "Pure-function repair-completion + device-return record construction
  -- an append-only ICT-equipment-repair-shop book-of-record draft,
  closely modeled on `cloud-itonami-isic-9512`'s `commrepair.registry`
  (both use `:device/return`, matching the equipment-repair naming
  convention rather than `:item/return`).

  Like every sibling actor's registry, there is no single international
  check-digit standard for a repair-completion or device-return
  reference number -- every shop/jurisdiction assigns its own reference
  ...
  the same honest, non-fabricating discipline `ictrepair.facts` uses.
  ...
  shape, the SAME real-world concern (a claimed parts cost must equal
  parts-quantity x parts-unit-price) reapplied to ICT-equipment-repair
  tickets (storage drives, power supplies, motherboards, displays)
  rather than consumer-electronics, communication-equipment, household-
  appliance, furniture, watch/jewelry/bicycle or footwear/leather-
  goods repair tickets -- not claimed as new.

  This namespace is pure data + pure functions -- no I/O, no network
  call to any real repair-shop-management system. It builds the RECORD
  a shop would keep, not the act of completing a repair or returning a
  device itself (that is `ictrepair.operation`'s `:repair/complete`/
  `:device/return`, always human-gated -- see README `Actuation`)."
...
(defn parts-cost-matches-claim?
  ...
  is an honest, literal reuse of `leathergoods.registry`'s own check,
  not a new concept."
...
(defn register-repair-completion
  ...
  the RECORD a shop would keep. `ictrepair.governor` independently
...
(defn register-device-return
  "Validate + construct the DEVICE-RETURN registration DRAFT -- the
  shop's own legal act of returning a real device to the customer.
  Pure function -- does not touch any real repair-shop-management
  system; it builds the RECORD a shop would keep. `ictrepair.governor`
  independently re-verifies the ticket's own post-repair safety-test
  status and media-sanitization status (when applicable), and blocks a
  double-return of the same ticket, before this is ever allowed to
  commit."
  [ticket-id jurisdiction sequence]
  (when-not (and ticket-id (not= ticket-id ""))
    (throw (ex-info "device-return: ticket_id required" {})))
  (when-not (and jurisdiction (not= jurisdiction ""))
  ...
  (let [return-number (str (str/upper-case jurisdiction) "-RTN-" (zero-pad sequence 6))
    ...
    {"record" record "return_number" return-number
     "certificate" (unsigned-certificate "DeviceReturn" return-number return-number)}))
