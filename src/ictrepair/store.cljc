(ns ictrepair.store
  "SSoT for the ICT-equipment-repair actor, behind a `Store` protocol
  so the backend is a swap, not a rewrite -- the same seam every prior
  `cloud-itonami-isic-*` actor in this fleet uses, closely modeled on
  `cloud-itonami-isic-9523`'s `leathergoods.store`:
  ...
  (test/ictrepair/store_contract_test.clj), which is the whole point:
  the actor, the Repair Governor and the audit ledger never know which
  SSoT they run on.

  Like `repairshop.store`'s/`commrepair.store`'s/`applianceshop.
  store`'s/`furniture.store`'s/`specialtyrepair.store`'s/
  `leathergoods.store`'s own dual history, this actor has TWO
  actuation events (repair completion, device return) acting on the
  ...
  Beyond `leathergoods.store`'s own `brand-screening-of`, this store
  carries `media-screening-of` (media-sanitization/data-destruction
  confirmation status for a replaced/removed storage component) --
  the genuinely new concern this vertical adds, since ICT equipment
  repair routinely replaces failed storage media (hard drives, SSDs)
  that may still hold recoverable customer personal data, a real,
  distinct regulatory concern beyond `repairshop`/9521's, `commrepair`/
  9512's, `applianceshop`/9522's, `furniture`/9524's, `specialtyrepair`/
  9529's and `leathergoods`/9523's own catalogs (and structurally
  different from `commrepair`/9512's own `customer-data-consent-
  confirmed?`, which is about consent to ACCESS data DURING repair,
  not secure DESTRUCTION of a REMOVED storage component afterward).
  ...
  media-sanitization status, which repair was completed, which device
...
(defprotocol Store
  ...
  (media-screening-of [s ticket-id] "committed media-sanitization screening verdict for a ticket, or nil")
  (assessment-of [s ticket-id] "committed jurisdiction assessment, or nil")
  (ledger [s])
  (completion-history [s] "the append-only repair-completion history (ictrepair.registry drafts)")
  (return-history [s] "the append-only device-return history (ictrepair.registry drafts)")
...
(defn demo-data
  ...
  lifecycles (repair completion, device return) plus the two
  unconditional-eval screening findings, so the actor + tests run
  offline."
  []
  {:tickets
   {"ticket-1" {:id "ticket-1" :customer "Sakura Tanaka" :item "Laptop (screen replacement)" :item-type :laptop
                 :parts-quantity 1 :parts-unit-price 20 :claimed-parts-cost 20.0
                 :safety-test-passed? true
                 :involves-storage-replacement? false :media-sanitization-confirmed? false
                 :repair-completed? false :device-returned? false
                 :jurisdiction "JPN" :status :intake}
    "ticket-2" {:id "ticket-2" :customer "Atlantis Doe" :item "Desktop PC (power-supply replacement)" :item-type :desktop
                 :parts-quantity 1 :parts-unit-price 15 :claimed-parts-cost 15.0
                 :safety-test-passed? true
                 :involves-storage-replacement? false :media-sanitization-confirmed? false
                 :repair-completed? false :device-returned? false
                 :jurisdiction "ATL" :status :intake}
    "ticket-3" {:id "ticket-3" :customer "鈴木一郎" :item "Server (motherboard replacement)" :item-type :server
                 :parts-quantity 1 :parts-unit-price 100 :claimed-parts-cost 150.0
                 :safety-test-passed? true
                 :involves-storage-replacement? false :media-sanitization-confirmed? false
                 :repair-completed? false :device-returned? false
                 :jurisdiction "JPN" :status :intake}
    "ticket-4" {:id "ticket-4" :customer "田中花子" :item "Laptop (battery replacement)" :item-type :laptop
                 :parts-quantity 1 :parts-unit-price 30 :claimed-parts-cost 30.0
                 :safety-test-passed? false
                 :involves-storage-replacement? false :media-sanitization-confirmed? false
                 :repair-completed? false :device-returned? false
                 :jurisdiction "JPN" :status :intake}
    "ticket-5" {:id "ticket-5" :customer "佐藤次郎" :item "Desktop PC (hard-drive replacement)" :item-type :desktop
                 :parts-quantity 1 :parts-unit-price 45 :claimed-parts-cost 45.0
                 :safety-test-passed? true
                 :involves-storage-replacement? true :media-sanitization-confirmed? false
                 :repair-completed? false :device-returned? false
                 :jurisdiction "JPN" :status :intake}}})
...
(defrecord MemStore [a]
  ...
  (media-screening-of [_ id] (get-in @a [:media-screenings id]))
  ...
  (commit-record! [s {:keys [effect path value payload]}]
    (case effect
      ...
      :media-screening/set
      (swap! a assoc-in [:media-screenings (first path)] payload)
...
(defn seed-db
  ...
  (->MemStore (atom (assoc (demo-data)
                           :assessments {} :safety-screenings {} :media-screenings {}
...
(def ^:private schema
  ...
  Map/compound values (assessment/safety-screening/media-screening
  ...
  {:ticket/id                  {:db/unique :db.unique/identity}
   ...
   :media-screening/ticket-id  {:db/unique :db.unique/identity}
...
(defn- ticket->tx [{:keys [id customer item item-type parts-quantity parts-unit-price claimed-parts-cost
                          safety-test-passed? involves-storage-replacement? media-sanitization-confirmed?
  ...
  (cond-> {:ticket/id id}
    ...
    (some? involves-storage-replacement?)                        (assoc :ticket/involves-storage-replacement? involves-storage-replacement?)
    (some? media-sanitization-confirmed?)                          (assoc :ticket/media-sanitization-confirmed? media-sanitization-confirmed?)
...
(def ^:private ticket-pull
  [:ticket/id :ticket/customer :ticket/item :ticket/item-type :ticket/parts-quantity
   ...
   :ticket/involves-storage-replacement? :ticket/media-sanitization-confirmed?
   :ticket/repair-completed? :ticket/device-returned?
...
(defn- pull->ticket [m]
  (when (:ticket/id m)
    {:id (:ticket/id m) :customer (:ticket/customer m) :item (:ticket/item m)
     ...
     :involves-storage-replacement? (boolean (:ticket/involves-storage-replacement? m))
     :media-sanitization-confirmed? (boolean (:ticket/media-sanitization-confirmed? m))
...
(defrecord DatomicStore [conn]
  ...
  (safety-screening-of [_ id]
    (dec* (d/q '[:find ?p . :in $ ?tid
              ...
              (d/db conn) id)))
  (media-screening-of [_ id]
    (dec* (d/q '[:find ?p . :in $ ?tid
                :where [?k :media-screening/ticket-id ?tid] [?k :media-screening/payload ?p]]
              (d/db conn) id)))
  ...
  (commit-record! [s {:keys [effect path value payload]}]
    (case effect
      ...
      :media-screening/set
      (d/transact! conn [{:media-screening/ticket-id (first path) :media-screening/payload (enc payload)}])
