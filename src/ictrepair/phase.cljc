(ns ictrepair.phase
  "Phase 0->3 staged rollout -- the ICT-equipment-repair analog of
  `cloud-itonami-isic-9523`'s `leathergoods.phase`.
  ...
  acts this actor performs; both are always a human repair
  technician/shop owner's call. `ictrepair.governor`'s `:actuation/
  complete-repair`/`:actuation/return-device` high-stakes gate enforces
  the same invariant independently -- two layers, not one, agree on
  this. `:safety/screen`/`:media/screen` are likewise never auto-
  eligible, at any phase -- the same posture every sibling's screening
  op has. Like `repairshop.phase`'s/`commrepair.phase`'s/
  `applianceshop.phase`'s/`furniture.phase`'s/`specialtyrepair.
  phase`'s/`leathergoods.phase`'s (and every prior sibling's) phase 3
...
(def write-ops #{:ticket/intake :jurisdiction/assess :safety/screen
                 :media/screen :repair/complete :device/return})
...
(def phases
  ...
  {0 {:label "read-only"       :writes #{}                                                                                :auto #{}}
   ...
   2 {:label "assisted-assess" :writes #{:ticket/intake :jurisdiction/assess :safety/screen :media/screen}                :auto #{}}
...
(defn verdict->disposition
  "Map a Repair Governor verdict to a base disposition before the
  phase gate."
  [verdict]
