...
(deftest device-return-never-auto-at-any-phase
  (testing "structural invariant: no phase, now or in the future entries, auto-commits a real device return"
    (doseq [[n {:keys [auto]}] phase/phases]
      (is (not (contains? auto :device/return))
          (str "phase " n " must not auto-commit :device/return")))))

(deftest safety-and-media-screen-never-auto-at-any-phase
  (testing "screening carries no direct capital risk, but is still never auto-eligible, matching every sibling screening op in this fleet"
    (doseq [[n {:keys [auto]}] phase/phases]
      ...
      (is (not (contains? auto :media/screen))
          (str "phase " n " must not auto-commit :media/screen")))))
