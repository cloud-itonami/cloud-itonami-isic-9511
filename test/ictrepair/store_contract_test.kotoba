(ns ictrepair.store-contract-test
...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      ...
      (is (= 20.0 (:claimed-parts-cost (store/ticket s "ticket-1"))))
      (is (true? (:safety-test-passed? (store/ticket s "ticket-1"))))
      (is (false? (:involves-storage-replacement? (store/ticket s "ticket-1"))))
      (is (= 150.0 (:claimed-parts-cost (store/ticket s "ticket-3"))))
      (is (false? (:safety-test-passed? (store/ticket s "ticket-4"))))
      (is (true? (:involves-storage-replacement? (store/ticket s "ticket-5"))))
      (is (false? (:media-sanitization-confirmed? (store/ticket s "ticket-5"))))
      (is (false? (:repair-completed? (store/ticket s "ticket-1"))))
      (is (false? (:device-returned? (store/ticket s "ticket-1"))))
      (is (= ["ticket-1" "ticket-2" "ticket-3" "ticket-4" "ticket-5"]
             (mapv :id (store/all-tickets s))))
      (is (nil? (store/safety-screening-of s "ticket-1")))
      (is (nil? (store/media-screening-of s "ticket-1")))
  ...
  (doseq [[label s] (backends)]
    (testing label
      (testing "partial upsert merges, preserving untouched fields"
        ...
        (is (= "Sakura Tanaka" (:customer (store/ticket s "ticket-1"))))
        (is (= 20.0 (:claimed-parts-cost (store/ticket s "ticket-1"))) "unrelated field preserved"))
      (testing "assessment / safety-screening / media-screening payloads commit and read back"
        ...
        (store/commit-record! s {:effect :media-screening/set :path ["ticket-1"]
                                 :payload {:ticket-id "ticket-1" :verdict :not-applicable}})
        (is (= {:ticket-id "ticket-1" :verdict :not-applicable} (store/media-screening-of s "ticket-1"))))
      ...
      (testing "device return drafts a record and advances the return sequence"
        ...
        (is (= "device-return-draft" (get (first (store/return-history s)) "kind")))
        (is (true? (:device-returned? (store/ticket s "ticket-1"))))
...
(deftest datomic-empty-store-is-usable
  (let [s (store/datomic-store)]
    ...
    (is (zero? (store/next-completion-sequence s "JPN")))
    (is (zero? (store/next-return-sequence s "JPN")))
    (store/with-tickets s {"x" {:id "x" :customer "n" :item "d" :item-type :laptop
