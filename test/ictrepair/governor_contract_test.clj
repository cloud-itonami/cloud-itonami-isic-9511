(ns ictrepair.governor-contract-test
  "The governor contract as executable tests -- the ICT-equipment-
  repair analog of `cloud-itonami-isic-9521`'s `repairshop.governor-
  contract-test` (itself the electronics-repair analog of `cloud-
  itonami-isic-6512`'s `casualty.governor-contract-test`), and
  structurally closest to `cloud-itonami-isic-9523`'s `leathergoods.
  governor-contract-test`. The single invariant under test:

    RepairOps-LLM never completes a repair or returns a device the
    Repair Governor would reject, `:repair/complete`/`:device/return`
    NEVER auto-commit at any phase, `:ticket/intake` (no direct
...
(deftest parts-cost-mismatch-is-held
  (testing "a claimed parts cost that doesn't equal quantity x unit-price -> HOLD (honest reuse of repairshop/9521's, commrepair/9512's, applianceshop/9522's, furniture/9524's, specialtyrepair/9529's and leathergoods/9523's own check)"
    (let [[db actor] (fresh)
...
(deftest safety-test-not-passed-is-held-and-unoverridable
  (testing "a failed post-repair safety test on a ticket -> HOLD, and never reaches request-approval (honest reuse of repairshop/9521's, commrepair/9512's, applianceshop/9522's, furniture/9524's, specialtyrepair/9529's and leathergoods/9523's own check)"
    (let [[db actor] (fresh)
...
(deftest media-sanitization-unconfirmed-is-held-and-unoverridable
  (testing "an unconfirmed media-sanitization status on a storage-replacement ticket -> HOLD, and never reaches request-approval -- exercised via :media/screen DIRECTLY, not via an actuation op against an unscreened ticket -- the genuinely NEW check this vertical adds, the 70th unconditional-evaluation-discipline grounding overall and the SEVENTH conditional variant, and structurally DISTINCT from commrepair/9512's own customer-data-consent-unconfirmed check (consent to access vs. secure destruction of removed media) (see this actor's governor ns docstring / parksafety's ADR-2607071922 Decision 5 / eldercare's, museum's, conservation's, salon's, entertainment's, casework's, hospital's, facility's, school's, association's, leasing's, behavioral's, secondary's, card's, water's, telecom's, aerospace's, recovery's, consulting's, union's, congregation's, fab's, energy's, care's, navigator's, learning's, banking's, advertising's, polling's, research's, design's, nursing's, sports's, alliedhealth's, laundry's, holdco's, photo's, personalservice's, edsupport's, headoffice's, residential's, cultural's, reserve's, proserv's, sportsevent's, recreation's, sportsclub's, partyops's, memberorg's, commrepair's, applianceshop's, socialresearch's, bizassoc's, vocational's, training's, furniture's, specialtyrepair's and leathergoods's ADR-0001s)"
    (let [[db actor] (fresh)
          res (exec-op actor "t7" {:op :media/screen :subject "ticket-5"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:media-sanitization-unconfirmed} (-> (store/ledger db) first :basis)))
      (is (nil? (store/media-screening-of db "ticket-5")) "no clearance written"))))

(deftest media-screen-is-a-noop-when-no-storage-replacement
  (testing "the media-sanitization check is CONDITIONAL: a repair that does not replace/remove a storage component has no media-sanitization requirement at all"
    (let [[_db actor] (fresh)
          res (exec-op actor "t7b" {:op :media/screen :subject "ticket-1"} operator)]
...
(deftest device-return-always-escalates-then-human-decides
  (testing "a clean, fully-assessed, passed-safety-test ticket still ALWAYS interrupts for human approval -- actuation/return-device is never auto"
    (let [[db actor] (fresh)
          ...
          r1 (exec-op actor "t9" {:op :device/return :subject "ticket-1"} operator)]
      (is (= :interrupted (:status r1)) "pauses for human approval even when governor-clean")
...
(deftest device-return-double-return-is-held
  (testing "returning the same ticket's device twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          ...
          _ (exec-op actor "t11a" {:op :device/return :subject "ticket-1"} operator)
          _ (approve! actor "t11a")
          res (exec-op actor "t11" {:op :device/return :subject "ticket-1"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
